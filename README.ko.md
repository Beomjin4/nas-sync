...
- 🛟 **데이터 손실 0 충돌 처리** — 동시 수정이 발생해도 어떤 버전도
  사라지지 않습니다. 마지막 저장이 활성본이 되고, 충돌된 파일도 서버에
  보존되어 어느 기기에서든(또는 웹 콘솔에서) 백업 및 복구 가능
- 🗑 **휴지통** — 삭제된 파일은 NAS에 30일(설정 가능) 보관, 원클릭 복구
...
- NAS가 보관함의 원본을 복사하여 보관합니다.
- 클라이언트는 오프라인 중 수정을 시도해도 재연결 후 즉시 업로드됩니다.
- Obsidian이 꺼진 동안 다른 파일 도구(메모장)로 바꾸어도 Obsidian 실행 후 자동으로 동기화 됩니다.
...
**동작 방식:** **최초로 페어링한 기기**의 보관함 전체가 NAS에
업로드되며, 그것이 원본이 됩니다. 이후 페어링하는 기기들은 첫 연결 시
그 보관함을 즉시 내려받습니다.

> ⚠ **두 번째 기기부터는 빈 보관함에 페어링하세요.** 새로 페어링하는
> 기기의 보관함에 같은 경로의 파일이 이미 있으면, 첫 동기화 때 
> **서버 데이터로 덮어써집니다.** 서버에 없는 경로의 파일은 업로드되어
> 보존되지만, 경로가 겹치는 로컬 파일은 새 기기 쪽이 지워집니다.
> 확실하지 않으면 먼저 백업하세요.
...
**로컬 LAN 또는 로컬 VPN** 환경에서 사용하길 추천합니다.
